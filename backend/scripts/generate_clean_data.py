...
NUM_JOBS = 5
NUM_APPLICATIONS = 80
MAX_INTERVIEWS_PER_APPLICATION = 3
...
def generate_interviews(applications, final_stage_per_app):
    ...
    for app in applications:
        ...
        stage_order = STAGES.index(stage_name)
        if stage_order < 1:
            continue
        ...
        base_scheduled = datetime.strptime(app["application_date"], "%Y-%m-%d") + timedelta(days=random.randint(2, 14))
        for idx in range(num_interviews):
            scheduled = base_scheduled + timedelta(days=idx * random.randint(1, 3), hours=random.randint(8, 17))
            if idx == 0:
                interview_type = "Phone Screen"
            elif idx == num_interviews - 1 and stage_order >= 4:
                interview_type = random.choice(["Technical", "Coding Challenge", "Final Panel"])
            else:
            ...
            recommendation = random.choices(
                ["Strong Hire", "Hire", "Leaning No", "No Hire"],
                weights=[15, 35, 25, 25],
                k=1,
            )[0]
            interviews.append({
                ...
                "technical_score": round(random.uniform(1, 5), 1) if random.random() > 0.2 else None,
                "communication_score": round(random.uniform(1, 5), 1) if random.random() > 0.2 else None,
                "overall_score": round(random.uniform(1, 5), 1) if random.random() > 0.15 else None,
                "recommendation": recommendation,
                "feedback": fake.sentence() if random.random() > 0.25 else None
            })
            interview_id += 1
    return interviews
        ...
        if app_id in final_stage_per_app:
            ...
            if stage["final_stage"] in ["Offer", "Offer Accepted", "Joined"] and stage["final_outcome"] == "Passed":
                ...
                offers.append({
                    ...
                    "offered_role": random.choice(["Senior Engineer", "Sales Rep", "Product Manager", "Analyst"]),
                    ...
                    "offer_rejection_reason": random.choice(["Salary Negotiation", "Competing Offer", None, None]) if offer_status in {"Declined", "Expired"} else None
                })
                offer_id += 1
    return offers
...
if __name__ == "__main__":
    ...
    support_summary = {
        ...
        "applications_reaching_offer": sum(1 for app in applications if app["application_id"] in final_stage_per_app and final_stage_per_app[app["application_id"]]["final_stage"] in {"Offer", "Offer Accepted", "Joined"}),
        "applications_reaching_joined": sum(1 for row in onboarding if row["joining_status"] == "Joined"),
    }
    ...
    with open(os.path.join(os.path.dirname(__file__), "clean_dataset_summary.json"), "w", encoding="utf-8") as f:
        import json
